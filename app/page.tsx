...
const Background = ({ type }: { type: Tab }) => (
  <div className="fixed inset-0 -z-20 overflow-hidden pointer-events-none">
    ...
    <div className={`absolute inset-0 transition-colors duration-700 ${type === "DRINK" ? "bg-[#1A103C]" : "bg-[#F4F1EA]"
      }`} />

    {/* Blob 1 (Top Right) */}
    <motion.div
      animate={{
        background: type === "DRINK"
          ? "radial-gradient(circle, rgba(212, 175, 55, 0.15) 0%, rgba(212, 175, 55, 0) 70%)" // Gold
          : "radial-gradient(circle, rgba(217, 56, 30, 0.15) 0%, rgba(217, 56, 30, 0) 70%)", // Orange
        scale: [1, 1.1, 0.9, 1],
        x: [0, 50, -20, 0],
        y: [0, -50, 20, 0],
      }}
      transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }}
      className="absolute -top-[20%] -right-[20%] w-[80vw] h-[80vw] rounded-full blur-[60px]"
    />
    ...
    <motion.div
      animate={{
        background: type === "DRINK"
          ? "radial-gradient(circle, rgba(100, 50, 200, 0.15) 0%, rgba(100, 50, 200, 0) 70%)" // Purple
          : "radial-gradient(circle, rgba(74, 93, 35, 0.15) 0%, rgba(74, 93, 35, 0) 70%)", // Kale
        scale: [1, 1.1, 0.9, 1],
        x: [0, -50, 20, 0],
        y: [0, 50, -20, 0],
      }}
      transition={{ duration: 25, repeat: Infinity, ease: "easeInOut" }}
      className="absolute -bottom-[20%] -left-[20%] w-[70vw] h-[70vw] rounded-full blur-[60px]"
    />
  </div>
);
...
Generated by Su Chef 👨‍🍳
  ...
  return (
    <>
      <Background type={activeTab} />
      <main className={`min-h-screen w-full flex flex-col p-6 md:p-12 transition-colors duration-500 ${activeTab === "DRINK" ? "text-jazz-text" : "text-ink"}`}>
        {/* Navigation / Header */}
